...
from bootstrap4.utils import add_css_class, render_template_file
...
class BaseRenderer(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.horizontal_label_class = kwargs.get('horizontal_label_class', get_bootstrap_setting('horizontal_label_class'))
        self.horizontal_field_class = kwargs.get('horizontal_field_class', get_bootstrap_setting('horizontal_field_class'))
...
class FieldRenderer(BaseRenderer):
    ...
    def fix_clearable_file_input(self, html):
        ...
        """
        return '<div class="row"><div class="col-xs-12">{html}</div></div>'.format(
            html=html
        )

    def post_widget_render(self, html):
        if isinstance(self.widget, RadioSelect):
            html = self.list_to_class(html, 'form-check')
        elif isinstance(self.widget, CheckboxSelectMultiple):
            html = self.list_to_class(html, 'form-check')
        elif isinstance(self.widget, SelectDateWidget):
            html = self.fix_date_select_input(html)
        elif isinstance(self.widget, ClearableFileInput):
            self.widget.attrs['class'] += 'form-control-file'
